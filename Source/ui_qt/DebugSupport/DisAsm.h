...
class CDisAsm : public CVirtualMachineStateView
{
public:
	typedef Framework::CSignal<void(uint32)> FindCallersRequestedEvent;

	CDisAsm(QMdiSubWindow*, CVirtualMachine&, CMIPS*);
	virtual ~CDisAsm();
...
private:
	...
	virtual std::string GetInstructionDetailsText(uint32);
