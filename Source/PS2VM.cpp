    ...
    , m_otherProfilerZone(CProfiler::GetInstance().RegisterZone("OTHER"))
{
	...
	m_ee->m_os->OnRequestLoadExecutable.connect(std::bind(&CPS2VM::ReloadExecutable, this, std::placeholders::_1, std::placeholders::_2));
}
...
void CPS2VM::CreateGsHandlerImpl(const CGSHandler::FactoryFunction& factoryFunction)
{
	...
	m_ee->m_gs->OnNewFrame.connect(std::bind(&CPS2VM::OnGsNewFrame, this));
}
