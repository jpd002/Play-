...
CGSH_OpenGL::TEXTURE_INFO CGSH_OpenGL::PrepareTexture(const TEX0& tex0)
{
	...
	for(const auto& candidateFramebuffer : m_framebuffers)
	{
		        ...
		        candidateFramebuffer->m_psm == tex0.nPsm)
		{
			...
			if((framebufferOffset & (CGsPixelFormats::PAGESIZE - 1)) != 0)
				continue;
			...
			if(framebufferPageIndex >= framebufferPageCountX)
				continue;
...
GLuint CGSH_OpenGL::PalCache_Search(const TEX0& tex0)
{
	    ...
	    paletteIterator != m_paletteCache.end(); paletteIterator++)
	{
		...
		if(!palette->m_live)
			continue;
		if(CGsPixelFormats::IsPsmIDTEX4(tex0.nPsm) != palette->m_isIDTEX4)
			continue;
		if(tex0.nCPSM != palette->m_cpsm)
			continue;
		if(tex0.nCSA != palette->m_csa)
			continue;
...
GLuint CGSH_OpenGL::PalCache_Search(unsigned int entryCount, const uint32* contents)
{
	    ...
	    paletteIterator != m_paletteCache.end(); paletteIterator++)
	{
		...
		if(palette->m_texture == 0)
			continue;

		unsigned int palEntryCount = palette->m_isIDTEX4 ? 16 : 256;
		if(palEntryCount != entryCount)
			continue;

		if(memcmp(contents, palette->m_contents, sizeof(uint32) * entryCount) != 0)
			continue;
