...
void Decoder::Execute(void* context, CBitStream& stream)
{
	...
	while(1)
	{
		...
		{
		case STATE_GETMARKER:			goto Label_GetMarker;
		case STATE_SKIPMARKER:			goto Label_SkipMarker;		
		case STATE_GETCOMMAND:			goto Label_GetCommand;
		case STATE_GETEXTENSIONID:		goto Label_GetExtensionId;
		case STATE_SKIPBITS:			goto Label_SkipBits;
		case STATE_EXECUTESUBPROGRAM:	goto Label_ExecuteSubProgram;
		case STATE_FINISHEXECUTE:		goto Label_FinishExecute;
		default:						assert(0);
		}
